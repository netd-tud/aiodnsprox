...
async def _send_query(client, uri, method, query,
                      accept=coap.CONTENT_FORMAT_DNS_MESSAGE,
                      content_format=coap.CONTENT_FORMAT_DNS_MESSAGE):
    request = aiocoap.Message(
        code=method, uri=uri, accept=accept, content_format=content_format,
        payload=query,
    )
    return await client.request(request).response


async def _send_query_safe(client, uri, method, query,
                           accept=coap.CONTENT_FORMAT_DNS_MESSAGE,
                           content_format=coap.CONTENT_FORMAT_DNS_MESSAGE,
                           dns_var="dns"):
    ...
    return await _send_query(client, uri, method, query,
...
async def test_coap_proxy(dns_server, config):
    ...
    try:
        ...
        query = query.to_wire()
        await _test_dns_query(client, dns_server,
        ...
        coap_response = await _send_query_safe(
                client, f'coaps://[{proxy_bind[0]}]/dns-query', aiocoap.FETCH,
                query, content_format=0)
        assert coap_response.code == \
            aiocoap.numbers.codes.Code.UNSUPPORTED_CONTENT_FORMAT
        coap_response = await _send_query_safe(
                client, f'coaps://[{proxy_bind[0]}]/dns-query', aiocoap.FETCH,
                query, accept=0)
        assert coap_response.code == \
            aiocoap.numbers.codes.Code.NOT_ACCEPTABLE
        coap_response = await _send_query_safe(
                client, f'coaps://[{proxy_bind[0]}]/dns-query', aiocoap.GET,
                query, dns_var='foobar')
        assert coap_response.code == \
